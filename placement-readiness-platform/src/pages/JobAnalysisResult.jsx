...
export default function JobAnalysisResult() {
    ...
    useEffect(() => {
        ...
        if (data) {
            // Schema compatibility (handle old data vs new strict data)
            // strict schema uses 'finalScore'
            if (data.readinessScore && !data.finalScore) data.finalScore = data.readinessScore;
            // strict schema uses 'plan7Days'
            if (data.plan && !data.plan7Days) data.plan7Days = data.plan;
            // strict schema extractedSkills is object
            setResult(data);
        }
    }, []);
