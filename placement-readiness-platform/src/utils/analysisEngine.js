...
function extractSkills(text) {
    ...
    return { extracted, totalCount };
}

// Helper: Calculate Base Score (Stable)
function calculateBaseScore(text, company, role, extracted, totalCount) {
    let score = 35; // Start
...
function generateRoundMapping(intel, extracted) {
    ...
    const hasDSA = skills.some(s => ["DSA", "Algorithms", "Data Structures"].some(k => s.includes(k)));
...
export function analyzeJob(company, role, jdText) {
    const { extracted, totalCount } = extractSkills(jdText);
    const score = calculateBaseScore(jdText, company, role, extracted, totalCount);
