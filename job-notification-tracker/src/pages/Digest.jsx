...
export default function Digest() {
    ...
    useEffect(() => {
        ...
        // Load Recent Status Updates (Last 24 hours simulation or all for demo)
        const savedStatuses = JSON.parse(localStorage.getItem('jobTrackerStatus') || '{}');
        const updates = Object.entries(savedStatuses)
            .map(([id, data]) => {
                const job = jobs.find(j => j.id === parseInt(id));
                return job ? { ...job, ...data } : null;
            })
            .filter(u => u && u.status !== 'Not Applied') // Only show active statuses
