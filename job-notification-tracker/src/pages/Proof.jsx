...
export default function Proof() {
    ...
    useEffect(() => {
        ...
        if (savedChecklist) {
            ...
            const isComplete = Object.values(parsed).every(Boolean) && Object.keys(parsed).length >= 10;
            setChecklistComplete(isComplete);
        }
    }, []);

    useEffect(() => {
        // Check if Shipped (All links present + Checklist complete)
        const linksValid = links.lovable && links.github && links.deploy;
        setShipped(linksValid && checklistComplete);
